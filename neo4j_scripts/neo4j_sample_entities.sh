...
CURRENT_DIR=$(pwd)
LOG_FILE_PATH=$CURRENT_DIR/neo4j_sample_entities_log.txt
...
echo -e "[$(date +%T)] * Start neo4j entity sampling...\n"

# create directory for exported data, start database and run data export script
cp $CURRENT_DIR/sample_entities.cypher $(ws_find $WORKSPACE_NAME)/containers/$NEO4J_CONTAINER_NAME/var/lib/neo4j
enroot start --rw $NEO4J_CONTAINER_NAME bash << EOF
    mkdir exported_data
    bin/neo4j start
    sleep 180
    cat sample_entities.cypher | bin/cypher-shell -u neo4j -p $PASSWORD > query_output.txt
    bin/neo4j stop
EOF
echo -e "\n[$(date +%T)] * Entity sampling script output:\n"
