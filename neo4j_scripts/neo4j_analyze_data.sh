...
CURRENT_DIR=$(pwd)
LOG_FILE_PATH=$CURRENT_DIR/neo4j_analyze_data_log.txt
...
echo -e "[$(date +%T)] * Start neo4j data analysis...\n"

# start database and run data analysis script
cp $CURRENT_DIR/analyze_data.cypher $(ws_find $WORKSPACE_NAME)/containers/$NEO4J_CONTAINER_NAME/var/lib/neo4j
enroot start --rw $NEO4J_CONTAINER_NAME bash << EOF
    bin/neo4j start
    sleep 180
    cat analyze_data.cypher | bin/cypher-shell -u neo4j -p $PASSWORD > query_output.txt
    bin/neo4j stop
EOF
echo -e "\n[$(date +%T)] * Data analysis script output:\n"
